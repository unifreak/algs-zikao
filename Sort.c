 ...
 *     记录  36 25 48 27 65 25 43 58 76 32
 *     dk=3  .        .        .        .   <- 第一次外循环直接插入排序这些元素
 *     dk=3     .        .        .         <- 第二次外循环直接插入排序这些元素
 *     dk=3        .        .        .      <- 第三次外循环直接插入排序这些元素
 *     一趟排序完成
 *
 * 性能:
 ...
 * p.175: 快速排序 -- 一次划分
 *
 * 如排序      [47  33  61  82  72  11  25  47]
 * 基准x=47     i                       .<- j       j 向前扫描, 找较基准较小者
 *            [25  33  61  82  72  11  25  47]     找到之后 (25), 移动到 i 位置
 *             i     ->.               j           i 向后扫描, 找较基准较大者
 *            [25  33  61  82  72  11  61  47]     找到之后 (61), 移动到 j 位置
 *                     i           .<- j           ...如此反复
 *            [25  33  11  82  72  11  61  47]
 *                     i ->.       j
 *            [25  33  11  82  72  82  61  47]
 *                         ij                      直到 j<=i 为止
 *            [25  33  11] 47 [72  82  61  47]     此时 i 位置即基准数字的正确位置
 *                                                 把基准数字放入 i 位置. 
 *                                                 i 左边都比基准数字小, 右边都比基准数字大
 *                    
 */
...
void HeapSort(SeqList R, int n) {
    int i;
    // 对初始数组 1..n 建立大根堆
    for (i = n/2; i > 0; i--) { // 从倒数第二层开始筛选, 故 i=n/2
        Sift(R, i, n);
    }
    // 对 1..i 进行堆排序, 共 n-1 趟
    for (i = n; i > 1; i--) {
...
void MergePass(SeqList R, SeqList MR, int len, int n) {
    ...
    // 循环做两两合并
    //
    // 注意这里的边界条件, 以文件 {a, b, c, d, e} 为例, 此时 n=5, len 假设为 1
 ...
 *
 * 稳定的: 
 * 每两个有序子文件归并时, 若存在相同关键字的记录, 归并排序能使前一个子文件中同一个关键字被先复制.
 * 从而确保它们的相对次序不会改变.
 *
 * 性能:
